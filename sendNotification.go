...
func sendMessageBitcoin(bitcoinNow infoBitcoin,db *sql.DB,bot *tgbotapi.BotAPI,updateRate time.Duration){
	for ; ; {
...
func sendMessageAboutCostBitcoin(bitcoinNow infoBitcoin,db *sql.DB,bot *tgbotapi.BotAPI,updateRate time.Duration){
	for;;{
		time.Sleep(updateRate)
...
func sendMessageAboutChangeCostBitcoin(bitcoinNow infoBitcoin,db *sql.DB,bot *tgbotapi.BotAPI,updateRate time.Duration){
	for;;{
		time.Sleep(updateRate)
		//из бд получим юзеров, которым нужно отправить оповещение
